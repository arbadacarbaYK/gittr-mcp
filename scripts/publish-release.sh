#!/usr/bin/env bash
# Cut a release: test, build .mcpb, tag, push tag, attach MCPB to GitHub Release.
# Works with GITHUB_PLATFORM_TOKEN (no workflow scope needed).
set -euo pipefail

ROOT="$(cd "$(dirname "$0")/.." && pwd)"
cd "$ROOT"

TOKEN="${GITHUB_PLATFORM_TOKEN:-${GITHUB_TOKEN:-}}"
if [[ -z "$TOKEN" ]]; then
  echo "ERROR: set GITHUB_PLATFORM_TOKEN or GITHUB_TOKEN" >&2
  exit 1
fi

VERSION="$(node -p "require('./package.json').version")"
TAG="v${VERSION}"
REPO="arbadacarbaYK/gittr-mcp"

echo "==> gittr-mcp release ${TAG}"
npm test
npm run build:mcpb

MCPB="dist/gittr-mcp-${VERSION}.mcpb"
if [[ ! -f "$MCPB" ]]; then
  echo "ERROR: missing $MCPB" >&2
  exit 1
fi

if git rev-parse "$TAG" >/dev/null 2>&1; then
  echo "Tag $TAG already exists locally."
else
  git tag -a "$TAG" -m "gittr-mcp ${VERSION}"
fi

echo "==> pushing ${TAG} to origin"
git push "https://arbadacarbaYK:${TOKEN}@github.com/${REPO}.git" "$TAG" || true

RELEASE_JSON="$(curl -fsS -H "Authorization: Bearer $TOKEN" -H "Accept: application/vnd.github+json" \
  "https://api.github.com/repos/${REPO}/releases/tags/${TAG}" 2>/dev/null || true)"

if [[ -z "$RELEASE_JSON" || "$(echo "$RELEASE_JSON" | python3 -c "import sys,json; d=json.load(sys.stdin); print(d.get('message',''))" 2>/dev/null)" == *"Not Found"* ]]; then
  echo "==> creating GitHub release ${TAG}"
  RELEASE_JSON="$(curl -fsS -X POST -H "Authorization: Bearer $TOKEN" -H "Accept: application/vnd.github+json" \
    "https://api.github.com/repos/${REPO}/releases" \
    -d "{\"tag_name\":\"${TAG}\",\"name\":\"gittr-mcp ${VERSION}\",\"body\":\"See CHANGELOG.md\",\"draft\":false,\"prerelease\":false}")"
else
  echo "==> release ${TAG} already exists"
fi

UPLOAD_URL="$(echo "$RELEASE_JSON" | python3 -c "import sys,json; print(json.load(sys.stdin).get('upload_url','').replace('{?name,label}',''))")"
ASSET_NAME="gittr-mcp-${VERSION}.mcpb"

# Replace existing asset with same name if present
RELEASE_ID="$(echo "$RELEASE_JSON" | python3 -c "import sys,json; print(json.load(sys.stdin).get('id',''))")"
if [[ -n "$RELEASE_ID" ]]; then
  ASSET_ID="$(curl -fsS -H "Authorization: Bearer $TOKEN" "https://api.github.com/repos/${REPO}/releases/${RELEASE_ID}/assets" | \
    python3 -c "import sys,json; assets=json.load(sys.stdin); print(next((a['id'] for a in assets if a.get('name')=='${ASSET_NAME}'), ''))")"
  if [[ -n "$ASSET_ID" ]]; then
    echo "==> removing old ${ASSET_NAME} asset"
    curl -fsS -X DELETE -H "Authorization: Bearer $TOKEN" "https://api.github.com/repos/${REPO}/releases/assets/${ASSET_ID}" >/dev/null
  fi
fi

echo "==> uploading ${ASSET_NAME}"
curl -fsS -X POST -H "Authorization: Bearer $TOKEN" -H "Content-Type: application/octet-stream" \
  "${UPLOAD_URL}?name=${ASSET_NAME}" --data-binary @"${MCPB}" | \
  python3 -c "import sys,json; d=json.load(sys.stdin); print('Download:', d.get('browser_download_url', d))"

echo "==> done. Local copy: gittr-mcp-${VERSION}.mcpb"
