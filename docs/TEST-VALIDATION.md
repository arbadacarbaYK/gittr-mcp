...
**Default CI / `npm test`:** smoke exports, MCP SDK reachability, stdio `tools/list` introspect (no nsec), NIP-82 builder tests, Pages path/auth helpers, lockfile→OSV parse tests, plus happy-path **dry-run only** (no relays mutations, no merge, no close).
